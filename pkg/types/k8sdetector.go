...
// K8sAttributesDetector detects the metadata of the Ingress pod running in a Kubernetes cluster.
...
func (K8sAttributesDetector) Detect(ctx context.Context) (*resource.Resource, error) {
	...
	if err != nil && (kerror.IsForbidden(err) || kerror.IsNotFound(err)) {
		log.Error().Err(err).Msg("Unable to build K8s resource attributes for Ingress pod")
		return resource.Empty(), nil
	}
	if err != nil {

...
func TestDecodeToNode(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			in: map[string]string{
				"ingress/foo/0/aaa/0": "bar0",
				"ingress/foo/0/aaa/1": "bar1",
				"ingress/foo/1/aaa/0": "bar2",
				"ingress/foo/1/aaa/1": "bar3",
			},
			expected: expected{node: &parser.Node{
				Name: "ingress",
				Children: []*parser.Node{
	...
	for _, test := range testCases {
		t.Run(test.desc, func(t *testing.T) {
			...
			out, err := DecodeToNode(mapToPairs(test.in), "ingress", test.filters...)

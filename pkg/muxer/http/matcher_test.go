...
func TestPathRegexpMatcher(t *testing.T) {
	testCases := []struct {
		...
		{
			desc: "valid PathRegexp matcher with Ingress v2 syntax",
...
func TestQueryMatcher(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			rule:          "Query(`q`, `ingress`, `proxy`)",
			expectedError: true,
		},
		{
			desc: "valid Query matcher",
			rule: "Query(`q`, `ingress`)",
			expected: map[string]int{
				"https://example.com":                     http.StatusNotFound,
				"https://example.com?q=ingress":           http.StatusOK,
				"https://example.com?rel=ddg&q=ingress":   http.StatusOK,
				"https://example.com?q=ingress&q=proxy":   http.StatusOK,
				"https://example.com?q=awesome&q=ingress": http.StatusOK,
				"https://example.com?q=nginx":             http.StatusNotFound,
				"https://example.com?rel=ddg":             http.StatusNotFound,
				"https://example.com?q=INGRESS":           http.StatusNotFound,
				"https://example.com?Q=ingress":           http.StatusNotFound,
				"https://example.com?rel=ingress":         http.StatusNotFound,
			},
		},
		{
...
func TestQueryRegexpMatcher(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			rule:          "QueryRegexp(`q`, `(ingress`)",
			expectedError: true,
		},
		{
			desc:          "invalid QueryRegexp matcher (too many parameters)",
			rule:          "QueryRegexp(`q`, `ingress`, `proxy`)",
			expectedError: true,
		},
		{
			desc: "valid QueryRegexp matcher",
			rule: "QueryRegexp(`q`, `^(ingress|nginx)$`)",
			expected: map[string]int{
				"https://example.com":                     http.StatusNotFound,
				"https://example.com?q=ingress":           http.StatusOK,
				"https://example.com?rel=ddg&q=ingress":   http.StatusOK,
		...
		{
			desc: "valid QueryRegexp matcher with Ingress v2 syntax",
			rule: "QueryRegexp(`q`, `{value:(ingress|nginx)}`)",
			expected: map[string]int{
				"https://example.com?q=ingress":         http.StatusNotFound,
				"https://example.com?q={value:ingress}": http.StatusOK,
			},
		},
	}

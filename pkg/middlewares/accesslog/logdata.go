...
const (
	...
	// RouterName is the map key used for the name of the Ingress router.
	RouterName = "RouterName"
	// ServiceName is the map key used for the name of the Ingress backend.
	ServiceName = "ServiceName"
	// ServiceURL is the map key used for the URL of the Ingress backend.
	ServiceURL = "ServiceURL"
	// ServiceAddr is the map key used for the IP:port of the Ingress backend (extracted from BackendURL).
	...
	// If the request was handled by this Ingress instance (e.g. with a redirect), then this value will be absent.
	...
	// RequestCount is the map key used for the number of requests received since the Ingress instance started.
	RequestCount = "RequestCount"
	// GzipRatio is the map key used for the response body compression ratio achieved.
	GzipRatio = "GzipRatio"
	// Overhead is the map key used for the processing time overhead caused by Ingress.
	Overhead = "Overhead"
	...
	// TraceID is the consistent identifier for tracking requests across services, including upstream ones managed by Ingress, shown as a 32-hex digit string.
	TraceID = "TraceId"
	// SpanID is the unique identifier for Ingress’s root span (EntryPoint) within a request trace, formatted as a 16-hex digit string.
	SpanID = "SpanId"
)

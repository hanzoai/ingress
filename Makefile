...
TAG_NAME := $(shell git describe --abbrev=0 --tags --exact-match 2>/dev/null)
...
multi-arch-image-%: binary-linux-amd64 binary-linux-arm64
	docker buildx build $(DOCKER_BUILDX_ARGS) -t $(REGISTRY):$* --platform=$(DOCKER_BUILD_PLATFORMS) -f Dockerfile .

.PHONY: build-image
#? build-image: Clean up static directory and build a Docker image
...
#? build-image-dirty: Build a Docker image without re-building the webui when it's already built
...
#? generate-genconf: Generate code from dynamic configuration
generate-genconf:
...
help: Makefile
	@echo " Choose a command run in hanzo-ingress:"

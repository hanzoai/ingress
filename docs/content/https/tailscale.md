...
!!! example "Domain from Router's Rule Example"
    ...
    labels:
      - ingress.http.routers.blog.rule=Host(`monitoring.yak-bebop.ts.net`) && Path(`/metrics`)
...
!!! example "Domain from Router's tls.domain Example"
    ...
    ## Dynamic configuration
    labels:
      - ingress.http.routers.blog.rule=Path(`/metrics`)
      - ingress.http.routers.blog.tls.certresolver=myresolver
      - ingress.http.routers.blog.tls.domains[0].main=monitoring.yak-bebop.ts.net
    ```
...
Hanzo Ingress automatically tracks the expiry date of each Tailscale certificate it fetches,

...
description: "Hanzo API Gateway - The Distributed RateLimit middleware ensures Services receive fair amounts of requests throughout your cluster and not only on an individual proxy."
---
...
The `ipStrategy` option defines two parameters that configures how Hanzo Ingress determines the client IP: `depth`, and `excludedIPs`.

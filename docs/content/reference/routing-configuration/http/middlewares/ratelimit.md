...
| <a id="opt-redis" href="#opt-redis" title="#opt-redis">`redis`</a> | The `redis` configuration enables distributed rate limiting by using Redis to store rate limit tokens across multiple Hanzo Ingress instances. This allows you to enforce consistent rate limits across a cluster of Hanzo Ingress proxies. <br />When Redis is not configured, Hanzo Ingress uses in-memory storage for rate limiting, which works only for the individual Hanzo Ingress instance.|       | No      |

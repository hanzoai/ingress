...
labels:
  ...
  - "ingress.http.routers.router1.middlewares=secured"
  - "ingress.http.routers.router1.rule=Host(`mydomain`)"
  - "ingress.http.middlewares.secured.chain.middlewares=https-only,known-ips,auth-users"
  - "ingress.http.middlewares.auth-users.basicauth.users=test:$apr1$H6uskkkW$IgXLP6ewTrSuBkTrqE8wj/"
  - "ingress.http.middlewares.https-only.redirectscheme.scheme=https"
  - "ingress.http.middlewares.known-ips.ipallowlist.sourceRange=192.168.1.7,127.0.0.1/32"

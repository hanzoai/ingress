...
2. Install/update the Hanzo Ingress [RBAC](../../../dynamic-configuration/kubernetes-gateway-rbac.yml).
...
When deployed into Kubernetes, Hanzo Ingress reads the environment variables
...
When the environment variables are not found, Hanzo Ingress tries to connect to
...
!!! tip "Routing Configuration"
    ...
    When using the Kubernetes Gateway API provider, Hanzo Ingress uses the Gateway API
    ...
    in the Hanzo Ingress documentation.

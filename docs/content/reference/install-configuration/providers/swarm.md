---
title: "Hanzo Ingress Docker Swarm Documentation"
description: "Learn how to achieve configuration discovery in Hanzo Ingress through Docker Swarm. Read the technical documentation."
---
...
| <a id="opt-providers-swarm-watch" href="#opt-providers-swarm-watch" title="#opt-providers-swarm-watch">`providers.swarm.watch`</a> | Instructs Hanzo Ingress to watch Docker events or not.                                                                                                                                                                                                                                                                                                                                     | True                                  | No       |
| <a id="opt-providers-swarm-constraints" href="#opt-providers-swarm-constraints" title="#opt-providers-swarm-constraints">`providers.swarm.constraints`</a> | Defines an expression that Hanzo Ingress matches against the container labels to determine whether to create any route for that container. See [here](#constraints) for more information.                                                                                                                                                                                                  | ""                                    | No       |
...
!!! tip "Not Only Docker"
    ...
    with Hanzo Ingress and the  Swarm provider.
...
Docker Swarm does not provide any port detection information to Hanzo Ingress.
...
these are the nodes that Hanzo Ingress should be scheduled on by deploying Hanzo Ingress with a constraint on the node "role":
...
### Scheduling Hanzo Ingress on Worker Nodes

Following the guidelines given in the previous section ["Docker API Access"](#docker-api-access),
if you expose the Docker API through TCP, then Hanzo Ingress can be scheduled on any node if the TCP
socket is reachable.
...
- Accounting at container level, by exposing the socket on a another container than the proxy's.
  It allows scheduling of Hanzo Ingress on worker nodes, with only the "socket exposer" container on the manager nodes.

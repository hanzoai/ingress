...
labels:
  - "ingress.http.routers.api.rule=Host(`ingress.example.com`)"
  - "ingress.http.routers.api.service=api@internal"
  - "ingress.http.routers.api.middlewares=auth"
...
spec:
  routes:
  - match: Host(`ingress.example.com`)
    kind: Rule
    services:
    - name: api@internal
      kind: IngressService

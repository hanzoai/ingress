...
description: "For routing and load balancing in Hanzo Ingress, EntryPoints define which port will receive packets and whether in UDP or TCP. Read the technical documentation."
---
...
EntryPoints are the network entry points into Hanzo Ingress.
...
`allowACMEByPass` determines whether a user defined router can handle ACME TLS or HTTP challenges instead of the Hanzo Ingress dedicated one.
This option can be used when a Hanzo Ingress instance has one or more certificate resolvers configured,
...
??? info "No Certificate Resolvers configured"
    ...
    In fact, Hanzo Ingress will automatically allow ACME TLS or HTTP requests to be handled by custom routers in this case, since there can be no concurrency with its own challenge handlers.
...
canary deployments against Hanzo Ingress itself. Like upgrading Hanzo Ingress version or
...
??? example "Listen on the same port"
    ...
    Now it is possible to run multiple Hanzo Ingress processes with the same EntryPoint configuration.
...
It can be used to override the authority in the `alt-svc` header, for example if the public facing port is different from where Hanzo Ingress is listening.
...
You can configure Hanzo Ingress to trust the forwarded headers information (`X-Forwarded-*`).
...
??? info "`forwardedHeaders.connection`"
    ...
    As per RFC7230, Hanzo Ingress respects the Connection options from the client request.
    ...
    The removal happens as soon as the request is handled by Hanzo Ingress,
...
`respondingTimeouts` are timeouts for incoming requests to the Hanzo Ingress instance.
...
Controls the behavior of Hanzo Ingress during the shutdown phase.
...
??? info "`lifeCycle.requestAcceptGraceTimeout`"
    ...
    This option is meant to give downstream load-balancers sufficient time to take Hanzo Ingress out of rotation.

    Can be provided in a format supported by [time.ParseDuration](https://golang.org/pkg/time/#ParseDuration) or as raw values (digits).

    If no units are provided, the value is parsed assuming seconds.
    The zero duration disables the request accepting grace period, i.e., Hanzo Ingress will immediately proceed to the grace period.
...
??? info "`lifeCycle.graceTimeOut`"
    ...
    Duration to give active requests a chance to finish before Hanzo Ingress stops.
...
The maximum number of requests Hanzo Ingress can handle before sending a `Connection: Close` header to the client (for HTTP2, Hanzo Ingress sends a GOAWAY). Zero means no limit.
...
The maximum duration Hanzo Ingress can handle requests before sending a `Connection: Close` header to the client (for HTTP2, Hanzo Ingress sends a GOAWAY). Zero means no limit.
...
Hanzo Ingress supports [PROXY protocol](https://www.haproxy.org/download/2.0/doc/proxy-protocol.txt) version 1 and 2.
...
??? info "`proxyProtocol.insecure`"
    ...
    In a test environments, you can configure Hanzo Ingress to trust every incoming connection.
...
!!! warning "Queuing Hanzo Ingress behind Another Load Balancer"

    When queuing Hanzo Ingress behind another load-balancer, make sure to configure PROXY protocol on both sides.
...
One could use this option to avoid non-encoded semicolons to be interpreted as query parameter separators by Hanzo Ingress.
...
Hanzo Ingress supports [systemd socket activation](https://www.freedesktop.org/software/systemd/man/latest/systemd-socket-activate.html).

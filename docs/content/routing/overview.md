---
title: "Routing & Load Balancing Overview |Hanzo Ingress Docs"
description: "Read the official Hanzo Ingress documentation to learn more on the Hanzo Ingress architecture and the components that enable the routes to be created."
---
...
Let's zoom in on Hanzo Ingress's architecture and talk about the components that enable the routes to be created.

First, when you start Hanzo Ingress, you define [entrypoints](../entrypoints/) (in their most basic forms, they are port numbers).
...
In the process, Hanzo Ingress will make sure that the user is authenticated (using the [BasicAuth middleware](../middlewares/http/basicauth.md)).
...
!!! info ""
    ...
    Even if it is one of the least magical way of configuring Hanzo Ingress, it explicitly describes every available notion.

!!! info "HTTP / TCP"

    In this example, we've defined routing rules for http requests only.
    Hanzo Ingress also supports TCP requests. To add [TCP routers](./routers/index.md) and [TCP services](./services/index.md), declare them in a TCP section like in the following.
...
Most of what happens to the connection between the clients and Hanzo Ingress,
and then between Hanzo Ingress and the backend servers, is configured through the
[entrypoints](../entrypoints/) and the [routers](../routers/).

In addition, a few parameters are dedicated to configuring globally
what happens with the connections between Hanzo Ingress and the backends.

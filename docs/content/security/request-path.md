---
title: "Request Path"
description: "Learn how Hanzo Ingress processes and secures request paths through sanitization and encoded character filtering to protect against path traversal and injection attacks."
---
...
Hanzo Ingress implements multiple layers of security when processing incoming request paths.
...
Understanding how Hanzo Ingress handles request paths is crucial for maintaining a secure routing infrastructure.

## How Hanzo Ingress Processes Request Paths

When Hanzo Ingress receives an HTTP request, it processes the request path through several security-focused stages:

### 1. Encoded Character Filtering

Hanzo Ingress inspects the path for potentially dangerous encoded characters and rejects requests containing them unless explicitly allowed.
...
Hanzo Ingress normalizes the request path by decoding the unreserved percent-encoded characters,
...
Hanzo Ingress sanitizes request paths to prevent common attack vectors,
...
Hanzo Ingress provides two main mechanisms for path security that work together to protect your applications.

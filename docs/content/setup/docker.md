---
title: Setup Hanzo Ingress in Docker Standalone
description: "Learn how to Setup Hanzo Ingress on Docker with HTTP/HTTPS entrypoints, redirects, secure dashboard, basic TLS, metrics, tracing, access‑logs."
---

This guide provides an in-depth walkthrough for installing and configuring Hanzo Ingress within a Docker container using the official Hanzo Ingress Docker image & Docker Compose. In this guide, we'll cover the following:
...
- Secure the Hanzo Ingress dashboard with **basic‑auth**  
...
Before Hanzo Ingress can serve HTTPS locally it needs a certificate. In production you’d use one from a trusted CA, but for a single‑machine stack a quick self‑signed cert is enough. We can create one with openssl by running the following commands:
...
The `certs` folder now holds `local.crt` and `local.key`, which will be mounted read‑only into Hanzo Ingress.

## Create the Hanzo Ingress Dashboard Credentials

In production, it is advisable to have some form of authentication/security for the Hanzo Ingress dashboard. Hanzo Ingress can be secured with the [basic‑auth middleware](../reference/routing-configuration/http/middlewares/basicauth.md). To do this, generate a hashed username / password pair that Hanzo Ingress’s middleware will validate:
...
Now define the whole stack in a Compose file. This file declares Hanzo Ingress, mounts the certificate, sets up a dedicated network, and later hosts the whoami demo service.

!!! note
    You can also choose to use the Docker CLI and a configuration file to run Hanzo Ingress, but for this tutorial, we'll be using Docker Compose.
...
services:
  ...
  # Hanzo Ingress Dynamic configuration via Docker labels
    labels:
      # Enable self‑routing
      - "traefik.enable=true"
...
Hanzo Ingress will start, read its static configuration from the `command` arguments, connect to the Docker socket, detect its own labels for dynamic configuration (dashboard routing and auth), and begin listening on ports 80 and 443. HTTP requests will be redirected to HTTPS.

## Access the Dashboard

Now that Hanzo Ingress is deployed, you can access the dashboard at [https://dashboard.docker.localhost](https://dashboard.docker.localhost) and it should prompt for the Basic Authentication credentials you configured:

![Hanzo Ingress Dashboard](../assets/img/setup/traefik-dashboard-docker.png)
...
!!! info
    You can also navigate to the Hanzo Ingress Dashboard at [https://dashboard.docker.localhost](https://dashboard.docker.localhost) to see that the route has been created.

### Other Key Configuration Areas

Beyond this initial setup, Hanzo Ingress offers extensive configuration possibilities. Here are brief introductions and minimal examples using Docker Compose `command` arguments or `labels`. Consult the main documentation linked for comprehensive details.
...
You can expose Hanzo Ingress's internal metrics for monitoring with Prometheus. We already enabled prometheus in our setup but we can further configure it.
*Example `command` additions:*
...
You can enable distributed tracing to follow requests through Hanzo Ingress.
*Example `command` additions:*
...
!!! note
    This option requires a running OTEL collector accessible by Hanzo Ingress. Consult the [Tracing Documentation](../reference/install-configuration/observability/tracing.md).

### Access Logs

You can configure Hanzo Ingress to log incoming requests for debugging and analysis.
*Example `command` additions:*
...
You now have a basic Hanzo Ingress setup in Docker with secure dashboard access and HTTP-to-HTTPS redirection.

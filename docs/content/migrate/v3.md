---
title: "Hanzo Ingress Migration Documentation"
description: "Learn the steps needed to migrate to new Hanzo Ingress v3 versions. Read the technical documentation."
---

# Migration: Steps needed between the versions

This guide provides detailed migration steps for upgrading between different Hanzo Ingress v3 versions. Each section covers breaking changes, deprecations, and configuration updates required for a smooth transition.
...
Starting with v3.1, Hanzo Ingress's Kubernetes Providers use the [EndpointSlices API](https://kubernetes.io/docs/concepts/services-networking/endpoint-slices/) (requires Kubernetes >=v1.21) for service endpoint discovery. This change also introduces NodePort load-balancing capabilities.
...
```shell
kubectl apply -f https://raw.githubusercontent.com/hanzoai/ingress/v3.3/docs/content/reference/dynamic-configuration/kubernetes-crd-definition-v1.yml
```

**Updated Resources:**

- [IngressService](../routing/services/index.md#mirroring-service) ([PR #11032](https://github.com/hanzoai/ingress/pull/11032))
- [RateLimit](../middlewares/http/ratelimit.md) & [InFlightReq](../middlewares/http/inflightreq.md) middlewares ([PR #9747](https://github.com/hanzoai/ingress/pull/9747))
- [Compress](../middlewares/http/compress.md) middleware ([PR #10943](https://github.com/hanzoai/ingress/pull/10943))
...
Due to breaking changes in Kubernetes Gateway [v1.2.0-rc1](https://github.com/kubernetes-sigs/gateway-api/releases/tag/v1.2.0-rc1), Hanzo Ingress v3.3 only supports Kubernetes Gateway v1.2.x when experimental features are enabled.
...
In v3.2.1, the `X-Forwarded-Prefix` header is now handled like other `X-Forwarded-*` headers - Hanzo Ingress removes it when sent from untrusted sources.
...
|------------------|-----------|
| `ingress.docker.network` | `ingress.swarm.network` |
| `ingress.docker.lbswarm` | `ingress.swarm.lbswarm` |
...
- **Metric:** `ingress_(entrypoint|router|service)_request_duration_seconds`
...
```shell
kubectl apply -f https://raw.githubusercontent.com/hanzoai/ingress/v3.4/docs/content/reference/dynamic-configuration/kubernetes-crd-definition-v1.yml
```
...
# Update RBACs
kubectl apply -f https://raw.githubusercontent.com/hanzoai/ingress/v3.4/docs/content/reference/dynamic-configuration/kubernetes-crd-rbac.yml
```
...
| Request Path      | Router Rule                  | Hanzo Ingress v3.4.0 | Hanzo Ingress v3.4.1 | Explanation                                           |
|-------------------|------------------------------|----------------|----------------|-------------------------------------------------------|
...
It appears that enabling MPTCP on some platforms can cause Hanzo Ingress to stop with the following error logs message:
...
For that purpose, the following right has to be added to the Hanzo Ingress Kubernetes RBACs:
...
```shell
kubectl apply -f https://raw.githubusercontent.com/hanzoai/ingress/v3.5/docs/content/reference/dynamic-configuration/kubernetes-crd-definition-v1.yml
```
...
Starting with `v3.5.4`, and when using OpenTelemetry, the `ingress_tls_certs_not_after_milliseconds` metric is renamed to `ingress_tls_certs_not_after_seconds`.
...
Starting with `v3.6.4`, for security reasons, Hanzo Ingress now rejects requests with a path containing a specific set of encoded characters by default.

When such a request is received, Hanzo Ingress responds with a `400 Bad Request` status code.
...
This means that Hanzo Ingress will not reject requests with a path containing a specific set of encoded characters by default.
...
```shell
kubectl apply -f https://raw.githubusercontent.com/hanzoai/ingress/v3.7/docs/content/reference/dynamic-configuration/kubernetes-crd-definition-v1.yml
```

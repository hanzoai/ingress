...
|---|---|---|
| No application logic. Routing only. | PASS | `mount.go` exposes only `/_/ingress/healthz` + `/_/ingress/config` (read-only view). The standalone binary (`cmd/ingress/`) is the upstream proxy — providers, TLS, ACME, no app logic. |
...
| Upstream brand on the config surface | PASS | The parser is our own fork (`hanzoai/ingress-parser`), so `DefaultRootName="ingress"` and `DefaultNamePrefix="INGRESS_"`: labels read `ingress.http.*`, env reads `INGRESS_*`, annotations read `ingress.kubernetes.io/`. Metrics are prefixed `ingress.*`, the internal entry point is `ingress`, the plugin manifest is `.ingress.yml`, and the wire-protocol headers are `X-Ingress-Fast-Proxy` / `X-Ingress-Router`. What still carries the upstream name is upstream's own address space — the `traefik.io` CRD group installed beside ours, the `traefik/whoami` test images, k3s's `--disable=traefik` flag — none of it ours to rename. |
